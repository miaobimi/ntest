CHAT = {
	...
	//第一个界面用户提交用户名
	usernameSubmit:function(uid,username){
		if(username != ""){
			this.init(uid,username);
		}
		return false;
	},
	init:function(uid,username){
		/*
		...
		*/
		...
		//连接websocket后端服务器
		this.socket = io.connect('ws://127.0.0.1:3002');
...
$(function(){
	var uid = CHAT.genUid();
	CHAT.usernameSubmit(uid,'游客'+uid);
